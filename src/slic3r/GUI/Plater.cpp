...
void Coordinates::set_coordinates_drop(wxCoord x, wxCoord y)
{
    coord_x = x;
    coord_y = y;
}
std::vector<wxCoord> Coordinates::get_coordinates_drop()
{
    std::vector<wxCoord> result;
    result.push_back(coord_x);
    result.push_back(coord_y);
    return result;
}
...
void Plater::set_idex_copy()
{
    //typedef std::set<unsigned int> IndicesList;
    //idex_copy = true;
    //GLVolumePtrs as= wxGetApp().plater()->canvas3D()->get_volumes().volumes;
    //as[0]->hover = as[0]->EHoverState::HS_Deselect;
    //remove(0);
    //GLVolumePtrs* ax= wxGetApp().plater()->canvas3D()->get_selection().m_volumes;
    //IndicesList* d =&(wxGetApp().plater()->canvas3D()->get_selection().get_volume_idxs());
    //wxGetApp().plater()->canvas3D()->get_selection().remove(1);
    //d.erase(0);

    //for (auto it = d.begin(); it != d.end(); ++it) { 
      //  wxGetApp().mainframe->output_through_framename(*it);
    //}
    //wxGetApp().mainframe->output_through_framename(as[1]->instance_idx());
    //wxGetApp().mainframe->output_through_framename(d[1]);

    //m_plater;
    //m_plater.load_files(wxGetApp().sidebar().obj_list()->object(0));
    //wxString a = wxGetApp().sidebar().obj_list()->object(0)[0].input_file;
    //wxGetApp().mainframe->plater()->load_files(a,LoadStrategy::LoadModel);

    //p->q->m_job_prepare_state;
    //p->q->load_model_objects(modelx[0].objects, false);
    //wxGetApp().mainframe->plater()->p.q;
    //ModelObject a;
    ...
    : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxSize(42 * wxGetApp().em_unit(), -1)), p(new priv(parent))
{
    ...
    // add printer
    {
        ...
        p->m_text_printer_settings = new Label(p->m_panel_printer_title, _L("Printer"), LB_PROPAGATE_MOUSE_EVENT); //Meta3D: Machine slide window
...
// Plater / private
//Meta3D: Class definition of priv
struct Plater::priv
{
    ...
    MenuFactory menus;
    //Meta3D: Vector that holds the location of previous instances
    std::vector<Vec3d> offset_vector;
...
bool PlaterDropTarget::OnDropFiles(wxCoord x, wxCoord y, const wxArrayString &filenames)
{
    ...
    //Meta3D: When a object is dropped it is loaded into
...
void Plater::priv::select_view_3D(const std::string& name, bool no_slice)
{
    ...
    else if (name == "Preview") {//Meta3D: Set panel to preview
        BOOST_LOG_TRIVIAL(info) << "select preview";
...
void Plater::test(double d)
{
    //wxGetApp().mainframe->output_through_framename(d);
    //Plater* idex_p = wxGetApp().mainframe->plater();
    //idex_p->copy_instances.back()->set_offset({250 - idex_p->copy_instances.front()->get_offset().x(),idex_p->copy_instances.front()->get_offset().y(),10.0f});
}
...
//#endif
    ...
    if (current_panel == view3D) {
        ...
        if (wxGetApp().is_editor()) {
            ...
            //Meta3D: Slice the objects when panel is changed to "preview"
    ...
    wxString const& originfile)
{
    ...
    if (!res.empty() && (load_restore || !(strategy & LoadStrategy::Silence))) {
        ...
        if (using_exported_file()) {
            ...
            p->set_project_filename(filename); //Meta3D: Puts the filename after importing from command + O 
        }  
    }
...
void Plater::add_model(bool imperial_units, std::string fname)
{
    ...
    std::vector<fs::path> paths;
    if (fname.empty()) {
        wxGetApp().import_model(this, input_files); //get the path to obejtc
                ...
                wxCAPTION | wxCLOSE_BOX)
{
    ...
    StateColor btn_bg_green(std::pair<wxColour, int>(wxColour(255, 174, 0  ), StateColor::Pressed), std::pair<wxColour, int>(wxColour(255, 174, 0  ), StateColor::Hovered),
...
void Plater::add_file()
{
    ...
    case LoadFilesType::Single3MF:
    	...
    	break;
    //Meta3D: add file as stl file
    case LoadFilesType::SingleOther: {
        Plater::TakeSnapshot snapshot(this, snapshot_label);
        if (!load_files(paths, LoadStrategy::LoadModel, false).empty()) {
            //load_files(paths, LoadStrategy::LoadModel, false);
            if (get_project_name() == _L("Untitled") && paths.size() > 0) {
                p->set_project_filename(wxString::FromUTF8(paths[0].string()));
